...
export default function BlockGrid() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { const data = await apiFetch<BlockSummary[]>("/blocks?limit=6"); if (alive) setBlocks(data); }
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-w-5xl">
      {blocks.map((b) => {
        const pct = b.candidate_count ? Math.round((b.tested_count / b.candidate_count) * 100) : 0;
        return (
          <div key={b.id} className="rounded-xl bg-white border border-slate-200 p-4">
            ...
            <div className="text-xs text-slate-600">range [{b.start.toLocaleString()} … {(b.end_excl-1).toLocaleString()} )</div>
            ...
            <div className="mt-1 text-xs text-slate-500">coverage {pct}% • {b.tested_count}/{b.candidate_count}</div>
          </div>
        );
      })}
