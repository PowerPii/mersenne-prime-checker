...
export default function FloatingRunner() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(ACTIVE_EVENT, refresh as EventListener);
  }, []);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (m.p != null && m.pct != null) { setP(m.p); setPctP(m.pct); }
  ...
  function handleHide() {
    clearActiveBlock();
    setBlockId(null); setTested(null); setTotal(null); setP(null); setPctP(0);
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 w-80 rounded-xl bg-white border border-slate-200 shadow-xl p-4">
      <div className="flex items-center justify-between">
        <div className="font-medium">Block {blockId}-{blockId+1}M Status</div>
        <button onClick={stop} className="px-2 py-1 text-xs rounded bg-rose-600 text-white hover:bg-rose-500">Stop</button>
      </div>
      <div className="mt-2 text-xs text-slate-600">Coverage: <b>{cov}%</b> {tested!=null&&total!=null?`(${tested}/${total})`:""}</div>
      ...
      <div className="mt-3 text-xs">
        {p!=null ? <>Currently testing: <b>M{p}</b> — <b>{pctP}%</b></> : <span className="text-slate-500">Waiting…</span>}
      </div>
      ...
      <div className="mt-3 flex justify-end">
        <button onClick={handleHide} className="text-xs text-slate-500 hover:text-slate-700">Hide</button>
      </div>
    </div>
  );
}
