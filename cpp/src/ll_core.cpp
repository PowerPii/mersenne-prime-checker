...
LLResult ll_test(const LLConfig& cfg, ProgressCb cb) {
  ...
  if (p < 2) throw std::invalid_argument("p must be >= 2");
  if (!is_prime_exponent(p)) throw std::invalid_argument("exponent p must be prime");
  ...
  if (p == 2) {
    ...
    quick.is_prime = true;
    quick.engine_info = std::string("gmp:") + (::gmp_version ? ::gmp_version : "?")
                      + "; " + compiler_info()
                      + "; flags:native";
    return quick;
  }
  ...
  for (std::uint32_t i = 0; i < total_iters; ++i) {
    // Early exit: if previous iteration produced s==0 and we still have work, M_p is composite.
  ...
  // Cleanup
  mpz_clear(M); mpz_clear(s); mpz_clear(tmp); mpz_clear(hi);
  ...
  // Engine info
  out.engine_info = std::string("gmp:") + (::gmp_version ? ::gmp_version : "?")
                  + "; " + compiler_info()
                  + "; flags:native";
