...
inline constexpr std::uint32_t rotr(std::uint32_t x, int n) { return (x >> n) | (x << (32 - n)); }
...
static void sha256(const void* data, std::size_t len, std::array<std::uint8_t,32>& out) {
    ...
    auto compress = [&](const unsigned char* b) {
        std::uint32_t w[64];
        for (int i = 0; i < 16; ++i) w[i] = load_be32(b + 4*i);
        for (int i = 16; i < 64; ++i) {
        std::uint32_t s0 = rotr(w[i-15],7) ^ rotr(w[i-15],18) ^ (w[i-15] >> 3);
        std::uint32_t s1 = rotr(w[i-2],17) ^ rotr(w[i-2],19) ^ (w[i-2] >> 10);
        w[i] = w[i-16] + s0 + w[i-7] + s1;
        }
        std::uint32_t a=H[0],b2=H[1],c=H[2],d=H[3],e=H[4],f=H[5],g=H[6],h=H[7];
        for (int i = 0; i < 64; ++i) {
    ...
    // Process full blocks
    for (std::size_t i = 0; i < full; i += 64) compress(in + i);

    // Padding
    std::size_t rem = len - full;
    std::memset(block, 0, 64);
    if (rem) std::memcpy(block, in + full, rem);
    block[rem] = 0x80;
    ...
    for (int i = 0; i < 8; ++i) block[56 + 7 - i] = (unsigned char)(bits >> (8 * i));
    compress(block);

    // Output
    for (int i = 0; i < 8; ++i) store_be32(out.data() + 4*i, H[i]);
}
