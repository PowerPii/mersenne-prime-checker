...
import asyncio, hashlib, math, os, sys, pathlib
...
import llcore  # type: ignore # pybind11 module
...
async def create_digits(req: Request, body: dict):
    p = int(body.get("p", 0))
    if p < 1: raise HTTPException(400, detail="p must be >= 1")
    ...
    if est_digits > MAX_DIGITS:
        raise HTTPException(413, detail=f"Requested decimal artifact is too large ({est_digits} digits).")
    ...
    def work():
        try:
            ...
            dao.artifact_insert(conn, job_id, filename, str(out_path), int(meta["digits"]), int(size_bytes), digest)
...
async def download_digits(req: Request, job_id: str):
    ...
    conn = req.app.state.db
    a = dao.artifact_get_by_job(conn, job_id)
    if not a: raise HTTPException(404, detail="artifact not ready")
    path = Path(a["path"])
    if not path.exists(): raise HTTPException(404, detail="file missing")
